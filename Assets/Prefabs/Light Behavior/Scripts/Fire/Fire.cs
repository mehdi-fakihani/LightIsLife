...
public class Fire : LightBehavior {
	 ...
	 *
	 * ChangeValue is depended on TimePassed, Calm Mode: ChangeValue = -Sin(TimePassed * 8 * pi - 1.5) * 0.03 + 0.97
	 * ChangeValue is depended on TimePassed, Intense Mode: ChangeValue = -Sin(TimePassed * 8 * pi -1.5) * 0.07 + 0.93
	 * **/
	...
	private float UpdateLightSource()
	{
		...
		if (TimePassed >= changeFireOn) {
			...
			//Intense Mode
			// Y = -Sin(X * 8 * pi - 1.5) * 0.07 + 0.93
			ChangeValue = -Mathf.Sin(TimePassed * 8 * Mathf.PI - 1.5f) * 0.07f + 0.93f;
			...
			//Calm Mode
			// Y = -Sin(X * 8 * pi - 1.5) * 0.03 + 0.97
			ChangeValue = -Mathf.Sin (TimePassed * 8 * Mathf.PI - 1.5f) * 0.03f + 0.97f;
		}
